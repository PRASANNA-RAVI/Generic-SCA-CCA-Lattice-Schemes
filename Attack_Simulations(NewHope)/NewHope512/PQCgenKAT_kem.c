...
// generate_first_cts
// resolve_conflicts_and_compute_complexity
...
int main(int argc, char *argv[])
{
  ...
  int prev_bit_array[(2*NEWHOPE_K+1)*(2*NEWHOPE_K+1)];
    ...
    // chosen ciphertexts.... 100 is much larger compared to the actual number of conflicted candidates in a cluster...
    ...
    {
        // Four coefficients (v0, v1) for the polynomial v are randomly selected from the chosen_v where v is of the structure
        // v = v0 . x^0 + v1 . x^256... We are also selecting one value for the coefficient of u where u = u0 . x^0
        ...
        // Setting initial values of all entries = 0 in decrypt_success_matrix...

        for(int iiii = 0;iiii < (2*NEWHOPE_K+1)*(2*NEWHOPE_K+1); iiii++)
            decrypt_success_matrix[no_trials][iiii] = 0;

        // for every possible candidate tuple (s0, s1), we calculate what will be the corresponding bit of the message based on the chosen values of (u0, v0, v1)...
        // Then we store it in each entry of the decrypt_success_matrix...
        // This will fill up one row of 1s and 0s in decrypt_success_matrix...
    ...
    sprintf(bits_values,"bits_values_resolved.dat");
    FILE * fbits_resolved = fopen(bits_values, "w+");
    fclose(fbits_resolved);

    sprintf(bits_values,"bits_values.dat");
    FILE * fbits = fopen(bits_values, "r");

    int bits_values_temp;

    for(int ppp=0;ppp<MAX_TRIALS;ppp++)
    {
        for(int pp=0;pp<(2*NEWHOPE_K+1)*(2*NEWHOPE_K+1);pp++)
        {
            fscanf(fbits, "%d,", &bits_values_temp);
            decrypt_success_matrix[ppp][pp] = bits_values_temp;
        }
        fscanf(fbits, " \n");
    }
    fclose(fbits);
    ...
    //     for(int pp=0;pp<(2*NEWHOPE_K+1)*(2*NEWHOPE_K+1);pp++)
    //     {
    //         printf("%d, ",decrypt_success_matrix[ppp][pp]);
    //     }
    //     printf("\n");
    // }
    ...
    for(col_1 = 0;col_1<no_collected;col_1++)
    {
        ...
        for(col_2 = 0;col_2<(2*NEWHOPE_K+1)*(2*NEWHOPE_K+1);col_2++)
        {
            ...
            if(same_sum == MAX_TRIALS)
            {
                choice1 = ((int)col_2/(2*NEWHOPE_K+1));
                choice2 = ((int)col_2%(2*NEWHOPE_K+1));
                collision_rows[col_1][sum] = col_2;
                sum++;
                prob_calc = prob_calc + (probabilities[choice1]*probabilities[choice2]);
            }
        }
    ...
    for(int row_no=0;row_no<no_collected;row_no++)
    {
        ...
        if(no_conflicted > 1)
        {
            ...
            int max_attempts = (no_chosen_u)*(64);
            int comb_no = 0;
            int u_count = 0;
            int v_1_count = 0;
            int v_2_count = 0;
            ...
            for(int tt = 0;tt<no_conflicted;tt++)
            {
                for(int qq = tt+1;qq<no_conflicted;qq++)
                {
                    ...
                    int choicee1 = ((int)current_conflicted_pair_rows[0]/(2*NEWHOPE_K+1)) - NEWHOPE_K;
                    int choicee2 = ((int)current_conflicted_pair_rows[0]%(2*NEWHOPE_K+1)) - NEWHOPE_K;
                    int choicee3 = ((int)current_conflicted_pair_rows[1]/(2*NEWHOPE_K+1)) - NEWHOPE_K;
                    int choicee4 = ((int)current_conflicted_pair_rows[1]%(2*NEWHOPE_K+1)) - NEWHOPE_K;
                    ...
                    {
                        // Iterating over all possible values of u0, v0, v1...
                            ...
                            for(v_1_count = 0;v_1_count < 8;v_1_count++)
                            {
                                for(v_2_count = 0;v_2_count < 8;v_2_count++)
                                {
                                    ...
                                    if(bits_matching_or_not == 1)
                                    {
                                        ...
                                        fprintf(f2,"%d, ",choice_u);
                                        fprintf(f2,"\n");

                                        f3 = fopen(v_1_values, "a");
                                        fprintf(f3, "%d *** %d\n",current_conflicted_pair_rows[0],current_conflicted_pair_rows[1]);
                                        fprintf(f3,"%d, ",v_1_count);
                                        fprintf(f3,"\n");

                                        f4 = fopen(v_2_values, "a");
                                        fprintf(f4, "%d *** %d\n",current_conflicted_pair_rows[0],current_conflicted_pair_rows[1]);
                                        fprintf(f4,"%d, ",v_2_count);
                                        fprintf(f4,"\n");

                                        fbits_resolved = fopen(bits_values, "a");
                                        fprintf(fbits_resolved, "%d *** %d\n",current_conflicted_pair_rows[0],current_conflicted_pair_rows[1]);
                                        fprintf(fbits_resolved,"%d, ", bits_match[0]);
                                        fprintf(fbits_resolved,"%d, ", bits_match[1]);
                                        fprintf(fbits_resolved,"\n");
    ...
    if(strcmp(argv[1], "run_attacks") == 0)
    {
        ...
        for(int no_attacks = 0 ; no_attacks < total_attacks; no_attacks++)
        {
            ...
            int u_value_now, v_1_value_now, v_2_value_now;

            uint8_t bits_oracle[MAX_TRIALS];
            int success = 0;
            int extra_trials = 0;

            // Secrets are retrieved two coefficients at a time... so for NewHope512, we need to do it 256 times...

            for(int secret_index = 0;secret_index < NEWHOPE_N/2; secret_index++)
            {
                FILE * f2 = fopen(u_values, "r");
                FILE * f3 = fopen(v_1_values, "r");
                FILE * f4 = fopen(v_2_values, "r");

                // Here, we run decapsulation of NewHope512 with the initially chosen MAX_TRIALS number of ciphertexts to narrow down the search space for the candidate tuple...
                ...
                for(int i = 0;i<MAX_TRIALS;i++)
                {
                    ...
                    crypto_kem_enc(sendb+8, key_b+8, pk+8, secret_index, u_value_now, v_1_value_now, v_2_value_now);
                ...
                int temp_bit_value;
                int bit_values_now[MAX_TRIALS];
                int no_matches = 0;
                int matching_secret_combo[MAX_CLASHES];
                for(int j = 0;j < (2*NEWHOPE_K+1)*(2*NEWHOPE_K+1); j++)
                {
                    ...
                    for(int i = 0;i<MAX_TRIALS;i++)
                    {
                        for(int k = 0;k < j; k++)
                        {
                            fscanf(f1, "%d, ",&temp_bit_value);
                        }
                        fscanf(f1, "%d, ",&bit_values_now[i]);
                        for(int k = 0;k < ((2*NEWHOPE_K+1)*(2*NEWHOPE_K+1) - j - 1); k++)
                        {
                            fscanf(f1, "%d, ",&temp_bit_value);
                        }
                        fscanf(f1, "\n");
                    ...
                    for(int i = 0;i<MAX_TRIALS;i++)
                    {
                        if(bits_oracle[i] == bit_values_now[i])
                            sum_matching++;
                    }
                    if(sum_matching == MAX_TRIALS)
                    {
                        matching_secret_combo[no_matches] = j;     // Exit when a matching candidate tuple is found...
                        no_matches++;
                    }
                }
                ...
                {
                    choice1 = ((int)matching_secret_combo[0]/(2*NEWHOPE_K+1)) - NEWHOPE_K;
                    choice2 = ((int)matching_secret_combo[0]%(2*NEWHOPE_K+1)) - NEWHOPE_K;
                    int coeff1, coeff2;

                    if(secret_index == 0)
                    {
                        if((choice1 == s_coeffs[0]) && (choice2 == s_coeffs[(NEWHOPE_N/2)]))
                        {
                            coeff1 = s_coeffs[0];
                            coeff2 = s_coeffs[(NEWHOPE_N/2)];
                            success++;

                            // Saving the guessed coefficients in guessed_s_coeffs...

                            guessed_s_coeffs[0] = coeff1;
                            guessed_s_coeffs[(NEWHOPE_N/2)] = coeff2;
                        }
                    }
                    else if(secret_index > 0)
                    {
                        if(choice1 == -1*(s_coeffs[NEWHOPE_N-secret_index]) && choice2 == s_coeffs[(NEWHOPE_N/2)-secret_index])
                        {
                            coeff1 = -1*(s_coeffs[NEWHOPE_N-secret_index]);
                            coeff2 = s_coeffs[(NEWHOPE_N/2)-secret_index];
                            success++;

                            // Saving the guessed coefficients in guessed_s_coeffs...

                            guessed_s_coeffs[NEWHOPE_N-secret_index] = -1*coeff1;
                            guessed_s_coeffs[(NEWHOPE_N/2)-secret_index] = coeff2;
                        }
                    }
                ...
                else if(no_matches > 1)
                {
                    ...
                    sprintf(bits_values_resolved,"bits_values_resolved.dat");
                    ...
                    int no_stages = ceil(log2(no_matches));
                    ...
                    choice1 = ((int)matching_secret_combo[bits_resolve_positions[0]]/(2*NEWHOPE_K+1)) - NEWHOPE_K;
                    choice2 = ((int)matching_secret_combo[bits_resolve_positions[0]]%(2*NEWHOPE_K+1)) - NEWHOPE_K;
                    int coeff1, coeff2;
            ...
            trace_complexity = trace_complexity + (float)(extra_trials + (NEWHOPE_N/2)*(MAX_TRIALS));
            success_rate = success_rate + (float)success/(NEWHOPE_N/2);
        ...
        printf("Avg. Trace requirement: %f, Success_Rate: %f",trace_complexity,success_rate);
    }
