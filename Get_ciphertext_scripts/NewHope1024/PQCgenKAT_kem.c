...
int main(void)
{
  ...
  uint8_t **decrypt_success_matrix = (uint8_t **)malloc(MAX_TRIALS * sizeof(uint8_t*));
  for(int i = 0; i < MAX_TRIALS; i++)
  {
    decrypt_success_matrix[i] = (uint8_t *)malloc((2*NEWHOPE_K+1)*(2*NEWHOPE_K+1)*(2*NEWHOPE_K+1)*(2*NEWHOPE_K+1) * sizeof(uint8_t));
  }

  // uint8_t decrypt_success_matrix[MAX_TRIALS][(2*NEWHOPE_K+1)*(2*NEWHOPE_K+1)*(2*NEWHOPE_K+1)*(2*NEWHOPE_K+1)];

  for(i=0;i<MAX_TRIALS;i++)
  {
      for(j=0;j<(2*NEWHOPE_K+1)*(2*NEWHOPE_K+1)*(2*NEWHOPE_K+1)*(2*NEWHOPE_K+1);j++)
      {
  ...
  int prev_bit_array[(2*NEWHOPE_K+1)*(2*NEWHOPE_K+1)*(2*NEWHOPE_K+1)*(2*NEWHOPE_K+1)];
    ...
    int greatest_sum = 0;
    int MAX_CLASHES=50;
    int max_no_collected =(2*NEWHOPE_K+1)*(2*NEWHOPE_K+1)*(2*NEWHOPE_K+1)*(2*NEWHOPE_K+1);
    ...
    while(flag == 1)
    {
        ...
        while(no_trials < MAX_TRIALS)
        {
            ...
            while(chose_u == 0)
            {
                ...
                for(choice11 = -1*(NEWHOPE_K);choice11<=(NEWHOPE_K);choice11++)
                {
                    ...
                    {
                        for(choice31 = -1*(NEWHOPE_K);choice31<=(NEWHOPE_K);choice31++)
                        {
                            for(choice41 = -1*(NEWHOPE_K);choice41<=(NEWHOPE_K);choice41++)
                            {
                                ...
                                temp3 = ((choice_u*choice31) + 3*NEWHOPE_Q) % NEWHOPE_Q;
                                if(temp3 < 0)
                                    temp3 = temp3 + NEWHOPE_Q;

                                temp4 = ((choice_u*choice41) + 3*NEWHOPE_Q) % NEWHOPE_Q;
                                if(temp4 < 0)
                                ...
                                t = ((t - NEWHOPE_Q));
                                t >>= 15;
                                bit_array[(choice41+(NEWHOPE_K))+(choice31+(NEWHOPE_K))*(2*NEWHOPE_K+1)+(choice21+(NEWHOPE_K))*(2*NEWHOPE_K+1)*(2*NEWHOPE_K+1)+(choice11+(NEWHOPE_K))*(2*NEWHOPE_K+1)*(2*NEWHOPE_K+1)*(2*NEWHOPE_K+1)] = t;
                                if(t == 1)
                                    flag_u = 1;
                            }
                        }
                ...
                for(choice2 = -1*(NEWHOPE_K);choice2<=(NEWHOPE_K);choice2++)
                {
                    for(choice3 = -1*(NEWHOPE_K);choice3<=(NEWHOPE_K);choice3++)
                    {
                        for(choice4 = -1*(NEWHOPE_K);choice4<=(NEWHOPE_K);choice4++)
                        {
                                ...
                                t >>= 15;
                                decrypt_success_matrix[no_trials][(choice4+(NEWHOPE_K))+(choice3+(NEWHOPE_K))*(2*NEWHOPE_K+1)+(choice2+(NEWHOPE_K))*(2*NEWHOPE_K+1)*(2*NEWHOPE_K+1)+(choice1+(NEWHOPE_K))*(2*NEWHOPE_K+1)*(2*NEWHOPE_K+1)*(2*NEWHOPE_K+1)] = t;
                                // printf("%d, ",t);
                        }
                    }
                }
            ...
            int count_ones = 0;
            int count_zeros = 0;
            for(int oo=0;oo<(2*NEWHOPE_K+1)*(2*NEWHOPE_K+1)*(2*NEWHOPE_K+1)*(2*NEWHOPE_K+1);oo++)
            {
                // printf("*** %d\n",decrypt_success_matrix[no_trials][oo]);
            ...
            if(count_zeros == (2*NEWHOPE_K+1)*(2*NEWHOPE_K+1)*(2*NEWHOPE_K+1)*(2*NEWHOPE_K+1))
            {
                if(flag_all_zeros == 0)
                    flag_all_zeros = 1;
            ...
            if(count_ones == (2*NEWHOPE_K+1)*(2*NEWHOPE_K+1)*(2*NEWHOPE_K+1)*(2*NEWHOPE_K+1))
            {
                if(flag_all_ones == 0)
                    flag_all_ones = 1;
            ...
            if(no_trials >= 1)
            {
                ...
                {
                    int count_same_2 = 0;
                    for(int pp=0;pp<(2*NEWHOPE_K+1)*(2*NEWHOPE_K+1)*(2*NEWHOPE_K+1)*(2*NEWHOPE_K+1);pp++)
                    {
                        if(decrypt_success_matrix[oo][pp] == decrypt_success_matrix[no_trials][pp])
                        {
                            // printf("%d, %d\n",decrypt_success_matrix[oo][pp],decrypt_success_matrix[no_trials][pp]);
                            count_same_2 = count_same_2+1;
                        }
                        if(count_same_2 == (2*NEWHOPE_K+1)*(2*NEWHOPE_K+1)*(2*NEWHOPE_K+1)*(2*NEWHOPE_K+1))
                        {
                            flag_same = 1;
            ...
            for(int pp=0;pp<(2*NEWHOPE_K+1)*(2*NEWHOPE_K+1)*(2*NEWHOPE_K+1)*(2*NEWHOPE_K+1);pp++)
                fprintf(f6,"%d, ", decrypt_success_matrix[no_trials][pp]);
            fprintf(f6,"\n");
            fclose(f6);
        ...
        uint8_t **unique_rows_collection = (uint8_t **)malloc((2*NEWHOPE_K+1)*(2*NEWHOPE_K+1)*(2*NEWHOPE_K+1)*(2*NEWHOPE_K+1) * sizeof(uint8_t*));
        for(int i = 0; i < (2*NEWHOPE_K+1)*(2*NEWHOPE_K+1)*(2*NEWHOPE_K+1)*(2*NEWHOPE_K+1); i++)
        {
          unique_rows_collection[i] = (uint8_t *)malloc(MAX_TRIALS * sizeof(uint8_t));
        }

        // int unique_rows_collection[(2*NEWHOPE_K+1)*(2*NEWHOPE_K+1)*(2*NEWHOPE_K+1)*(2*NEWHOPE_K+1)][MAX_TRIALS];
        no_collected = 0;
