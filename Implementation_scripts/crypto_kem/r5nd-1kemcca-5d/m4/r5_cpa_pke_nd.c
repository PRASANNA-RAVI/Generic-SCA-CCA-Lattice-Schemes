...
static void r5_pack_q_p_u(uint8_t *pv, const modq_t *vq,
    const modq_t rounding_constant, int count, int profiling)
{
...
int r5_cpa_pke_encrypt(uint8_t *ct, const uint8_t *pk,
    const uint8_t *m, const uint8_t *rho, int count, int profiling)
{
    size_t i, j;
    modq_t a[2 * (PARAMS_D + 1)];
    ...
    r5_pack_q_p_u(ct, vec.u_t, PARAMS_H2, count, profiling); // ct = U^T | v
...
int r5_cpa_pke_encrypt_cmp(uint8_t *ct, const uint8_t *pk,
