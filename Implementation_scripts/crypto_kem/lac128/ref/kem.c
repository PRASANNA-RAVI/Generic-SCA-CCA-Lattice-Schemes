...
{
	kem_enc_fo(pk,ss,ct,count,profiling);
	return 0;
}
...
int kem_enc_fo(const unsigned char *pk, unsigned char *k, unsigned char *c, int count, int profiling)
{
	...
	//encrypt m with seed
	pke_enc_seed(pk,buf,MESSAGE_LEN,c,&clen,seed, count, profiling);
...
int kem_enc_fo_seed(const unsigned char *pk, unsigned char *k, unsigned char *c, unsigned char *seed)
{
	...
	pke_enc_seed(pk,buf,MESSAGE_LEN,c,&clen,local_seed, 0, 0);
...
int kem_dec_fo(const unsigned char *pk, const unsigned char *sk, const unsigned char *c, unsigned char *k)
{
	...
	gen_seed(buf,MESSAGE_LEN,seed);
	pke_enc_seed_cmp(pk,buf,MESSAGE_LEN,c_v,&clen,seed);
