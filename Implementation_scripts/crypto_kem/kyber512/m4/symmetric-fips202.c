...
void shake256_prf(unsigned char *output, size_t outlen, const unsigned char *key, unsigned char nonce) {
    ...
    // unsigned int address = extkey;
    // unsigned char char_address;
    //
    // char_address = (address&0xFF);
    // send_USART_bytes(&char_address,1);
    //
    // char_address = (address>>8)&0xFF;
    // send_USART_bytes(&char_address,1);
    //
    // char_address = (address>>16)&0xFF;
    // send_USART_bytes(&char_address,1);
    //
    // char_address = (address>>24)&0xFF;
    // send_USART_bytes(&char_address,1);
